...
bool VF2MonoState::IsFeasiblePair(node_id node1, node_id node2)
  { assert(node1<n1);
    ...
    for(i=0; i<g1->OutEdgeCount(node1); i++)
      { other1=g1->GetOutEdge(node1, i, &attr1); // other1 = out-edge id of an out-edge of node1, edge i
        if (core_1[other1] != NULL_NODE)  // if neighbor of g1 is already mapped
          { other2=core_1[other1];
            if (!g2->HasEdge(node2, other2) ||
                !g1->CompatibleEdge(attr1, g2->GetEdgeAttr(node2, other2)))	// and if the node in g2 doesn't have a mapped neighbor
              return false;
          }
        else    // else if out-edge of g1 doesn't connect to a mapped node
          { if (in_1[other1])   // if out-edge of g1 points to a G1 node with in-edges
              termin1++;
            if (out_1[other1]) // if out-edge of g1 points to a G1 node with out-edges
              termout1++;
            if (!in_1[other1] && !out_1[other1]) // if out-edge of g1points to g1 node with NO other in- or out-edges
              new1++;
          }
      }
    ...
    for(i=0; i<g2->OutEdgeCount(node2); i++)
      { other2=g2->GetOutEdge(node2, i);		// other2 = an out-edge of node2
        if (core_2[other2]!=NULL_NODE)			// if the node other2 points2 is already mapped then do nothing
          { /* Do nothing */
          }
        else 
          { if (in_2[other2])
